...
//		}
	...
	public synchronized static String takeScreenshot(WebDriver driver, String screenshotName) {
		...
		// Sanitize the name: replace all non-alphanumeric characters with underscores
		String sanitizedName = screenshotName.replaceAll("[^a-zA-Z0-9]", "_");

		TakesScreenshot ts = (TakesScreenshot) driver;
		File src = ts.getScreenshotAs(OutputType.FILE);
		String timeStamp = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String destPath = System.getProperty("user.dir") + "/src/test/resources/screenshots/" + sanitizedName + "_"
				+ timeStamp + ".png";
	...
	public synchronized static void attachScreenshot(WebDriver driver, String message) {
		try {
			if (driver != null && getTest() != null) {
				// Capture directly to memory string, skipping the hard drive
				String screenShotBase64 = takeScreenshotBase64(driver);
